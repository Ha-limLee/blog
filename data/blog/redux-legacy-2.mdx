...
export const IMAGE_PATH = '/static/images/redux-legacy-2';
...
<ImageFromStatic name={'remove-todo-type-error'} />
...
<ImageFromStatic name={'set-todo-type-error-with-union'} />
...
<ImageFromStatic name="switch-case-set-todo" />

<ImageFromStatic name="switch-case-remove-todo" />
...
<ImageFromStatic name="union-payload" />

이제 payload의 타입이 제대로 잡힌다

<ImageFromStatic name="type-assert-set-todo" />

<ImageFromStatic name="type-assert-remove-todo" />

`combineReducers`에서도 잘 나온다

`src/features/reducers.js`

<ImageFromStatic name="reducers-union-type" />

store에도 잘 적용되었다

`src/app/store.js`

<ImageFromStatic name="store-complete" />
