...
export const IMAGE_PATH = '/static/images/property-accessors-type';
...
<ImageFromStatic name="referencing-nullish" />
...
<ImageFromStatic name="property-access-code" />

타입 오류가 없다! value는 `number`로 추론된다

<ImageFromStatic name="property-access-no-undefined" />
...
<ImageFromStatic name="ts-config-no-unchecked-indexed-access" />

이 옵션을 활성화하면 속성 접근자로 가져온 값의 타입이 `T | undefined`가 된다

<ImageFromStatic name="property-access-with-config" />
...
<ImageFromStatic name="underscore-get" />

배열의 값만 가져온다면 ES2022에 추가된 `Array.prototype.at()`을 사용할 수도 있다[^j]

<ImageFromStatic name="array-at" />
...
<ImageFromStatic name="custom-get" />
