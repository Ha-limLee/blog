...
export const IMAGE_PATH = '/static/images/barrel-file-is-bad';
...
<ImageFromStatic name="jest-with-circular" />

위 코드는 순환참조가 있는 모듈을 참조한 테스트 코드다

이 코드를 실행하면 어떻게 될까?

<ImageFromStatic name="test-circular" />

OOM이 발생하고 테스트가 끝나지 않았다

그렇다면 직접참조하면 어떨까?

<ImageFromStatic name="jest-without-circular" />

경로를 utils에서 utils/colors로 바꿨다

<ImageFromStatic name="test-no-circular" />
