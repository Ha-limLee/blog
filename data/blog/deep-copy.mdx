...
[이전 글](/blog/denote-mutation)에서는 매개변수를 변경하는 비순수 함수를 조금 더 안전하게 만드는 방법에 대해서 알아보았다
...
속도가 중요하다면 검증된 라이브러리를 이용하거나 재귀함수를 이용해 깊은 복사를 해야한다
...
const recursiveClone = (obj) => {
  ...
  const cloned = obj.constructor(...obj)
  for (const key in obj) cloned[key] = recursiveClone(obj[key])
  return cloned
}
...
const cloned = recursiveClone(obj)
...
const copied = recursiveClone(sample)

console.timeEnd('recursiveClone') // recursion: 0.195ms
```
...
단순 재귀 복사는 JSON 방식보다 약 6425배 빠르다

JSON 방식은 `structuredClone`보다 약 2배 빠르다
